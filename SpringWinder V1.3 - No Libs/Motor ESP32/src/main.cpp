...
static const int ENABLE_PIN  = 12;   // set -1 if not used (LOW = enabled)
...
struct Config {
  ...
  // Convenience
  float steps_per_mm   = 400.0f;    // if you want to think in mm
...
//Used to check on the values in the matrix
void PrintMatrix (){                
 for (int i = 0; i < 10; ++i) {
        for (int j = 0; j < 2; ++j) {
        Serial.print(CalibValuesMatrix[i][j], 2); // 2 decimal places
        Serial.print("\t");            // tab space between columns
    }
    Serial.println();                // new line after each row
  }
}

void ClearLoadCellValues()
 {
    for (uint8_t i = 0; i < lc.getCalibrateSize(); i++) {
        lc.setCalibrate(i, 0, 0);  // reset to 0
    }

 	for (int i = 0; i < LC_INDEX_SIZE; i++) {
         CalibValuesMatrix[i][0] = 0.0f; // Known weight
         CalibValuesMatrix[i][1] = 0.0f; // Raw reading
     }
     PrintMatrix();
}
...
{
	ClearLoadCellValues();
  float reading = lc.read_average(5);
	lc.setCalibrate(0, reading, 0);         // raw, weight = 0
    CalibValuesMatrix[0][0] = 0;       // Known weight
    CalibValuesMatrix[0][1] = reading; // Raw reading
    Serial.println("Tare completed:");
    PrintMatrix();
}

void ApplyCalibration(){      //need to fill all empty slots i in the setCalibrate() function or calibration won't trigger
    int n = 0;

    for (int i = 0; i < LC_INDEX_SIZE; i++) {
        float w = CalibValuesMatrix[i][0];
        float r = CalibValuesMatrix[i][1];
        if (w != 0 || r != 0) { // skip empty slots
            n++;
            lc.setCalibrate(i, r, w);
            Serial.printf("Calibration applied: %.1f, raw=%.6f, weight=%.3f\n", i, r, w);
        }
    }
    if (n < 2) {
        Serial.println("Not enough points for calibration!");
        return;
    }
}

void AddCalibrationValue() {
    // Find first empty row after tare (index 1+)
    int slot = -1;	
    for (int i = 1; i < LC_INDEX_SIZE; ++i) {
        if (CalibValuesMatrix[i][0] == 0 && CalibValuesMatrix[i][1] == 0) {
            slot = i;
            break;
        }
    }
    if (slot == -1) {
        Serial.println("Calibration matrix is full!");
        return;
    }

    // Get known weight serial monitor
    float knownWeight = cfg.knownWeight;

    // Get load cell raw reading
    float reading = lc.read_average(5);

    // Store into matrix
    CalibValuesMatrix[slot][0] = knownWeight;
    CalibValuesMatrix[slot][1] = reading;

    Serial.printf("Added calibration point %d: weight=%.2f, raw=%.2f\n", slot, knownWeight, reading);
    PrintMatrix();
    ApplyCalibration();
}
...
float lc_read()
{
    float weight = lc.get_units(5);   // <-- returns weight after calibration
    return weight;
}
...
void handleCommand(const String &line) {
  ...
  if (s == "tare") {
    Tare_Load_Cell(); // do a few samples
    Serial.println(F("Tare done."));
    return;
  }
  
  if (s.startsWith("cal weight ")) {
    float kW = s.substring(11).toFloat();
    lockCfg();
    cfg.knownWeight = kW;
    AddCalibrationValue();
    unlockCfg();
    Serial.printf("HX weight -> %.6f\n", kW);
    return;
  }

  if (s.startsWith("lc preset")) {
    //float presetNum = s.substring(14).toFloat();
    lockCfg();
    CalibrateLCPreSet();
    unlockCfg();
    Serial.printf("Preset Added");
    return;
  }
...
void taskControl(void *pv) {
  ...
  for (;;) {
    ...
    // 2) Read HX711 and print status at rate
    uint32_t now = millis();
    if (now - lastPrint >= printIntervalMs) {
      lastPrint = now;

      // Get load cell reading (units depend on scale factor; treat as Newtons or lbf per your calibration)
      // Take a small average to smooth (non-blocking-ish)
      float reading = lc.get_units(3); // average of 3 samples

      // Position
      long pos = stepper.currentPosition();

      if (currentMode == Mode::LOADCELL) {
        Serial.printf("[LOADCELL] t=%lu ms, force=%.3f, pos=%ld\n",
                      (unsigned long)now, reading, pos);
      } else if (currentMode == Mode::LIVETEST) {
        // Stream during motion too
        Serial.printf("[LIVE] t=%lu ms, cyc=%lu/%lu, state=%s, %ld, %.3f\n",
                      (unsigned long)now,
